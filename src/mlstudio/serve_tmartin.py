...
uv run fastapi dev src/mlstudio/serve_tmartin.py      # development (auto-reload)
uv run fastapi run src/mlstudio/serve_tmartin.py      # production

- OR -

uv run uvicorn mlstudio.serve_tmartin:app --reload    # development (auto-reload)
uv run uvicorn mlstudio.serve_tmartin:app             # production
...
If macOS or Linux, use \ line continuation characters:
    ...
    curl -X POST http://127.0.0.1:8000/predict \
         ...
         -d '{"bill_length_mm": 34.4, "bill_depth_mm": 18.4, "flipper_length_mm": 184, "body_mass_g": 3325}'
...
app = FastAPI(title="Penguin island classifier")
